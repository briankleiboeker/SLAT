...
ui <- fluidPage(
  titlePanel("SLAT: Shotgun Lipidomics Assignment Tool"),
  tabsetPanel(
    id = "tabsid",
    tabPanel(value = 0,
  ...
  output$analyzed_data <- renderUI({
    ...
    if(!is.na(species.result)){
      if(species.result %in% pls$gen.structure){
        ...
        c <- unname(unlist(pls[ match(species.result,pls$gen.structure) , 2:ncol(pls) ]))
        c <- c[c != ""]
        
        if(length(c) <= 1){
          species.result <- paste0(species.result ,"</b>","; most likely exact structure: ", paste0(c,collapse = "") )
        }else{
          species.result <- paste0(species.result ,"</b>","; most likely exact structures: ", paste(c, collapse = ", ") )
        }
    ...
    if(input$comp == ""){
      ...
      tagList(
        ...
        p("Need to assign elemental compositions to m/z values after realigning m/z's? Check out ",url,"Instructions for usage, source code, and a test dataset are available at ",url2,""," For reproducible, high-throughput assignment of lipid class and structure, check out ",url3),
      ...
      HTML(paste("Report problems, suggestions, or other feedback to bkleiboeker [at] wustl.edu","","Instructions for usage, source code, and a test dataset are available at ",url3," ",str1, str2, str3, str4, str5, str6, str7,str8,str9,str10,str11,str12,sep = '<br/>'))
    }
    ...
    if(!is.null(imported$status())){
      if(imported$status() == "success"){
        if(!("composition" %in% colnames(data.full()))){
          ...
          HTML(paste0("Import successful. Detected columns: ",paste( colnames(data.full())[colnames(data.full()) != "structure" & !grepl("exact.structure",colnames(data.full()))],collapse = ', '),ifelse("delta" %in% colnames(data.full()),". Delta column detected, so filtering by abs(delta) cutoff feature is enabled. ",". Delta column not detected, so filtering by abs(delta) cutoff feature is disabled. "),ifelse( any(data.full()$structure %in% pls$gen.structure) ,""," Exact structure is unknown for all assigned structures, so exact structure columns are not being shown.")))
        }
      }else{
        imported$status()
      }
  ...
  data.full <- reactive({
    if(!is.null(imported$data())){
      ...
      if(any(df$structure %in% pls$gen.structure)){
        ...
        df$exact.structure.1 <- unlist(lapply(df$structure,function(x) ifelse(x %in% pls$gen.structure,unname(unlist(pls[match(x,pls$gen.structure),2:ncol(pls)]))[1],"" )) )
        df$exact.structure.2 <- unlist(lapply(df$structure,function(x) ifelse(x %in% pls$gen.structure,unname(unlist(pls[match(x,pls$gen.structure),2:ncol(pls)]))[2],"" )) )
        df$exact.structure.3 <- unlist(lapply(df$structure,function(x) ifelse(x %in% pls$gen.structure,unname(unlist(pls[match(x,pls$gen.structure),2:ncol(pls)]))[3],"" )) )
        df$exact.structure.4 <- unlist(lapply(df$structure,function(x) ifelse(x %in% pls$gen.structure,unname(unlist(pls[match(x,pls$gen.structure),2:ncol(pls)]))[4],"" )) )
        df$exact.structure.5 <- unlist(lapply(df$structure,function(x) ifelse(x %in% pls$gen.structure,unname(unlist(pls[match(x,pls$gen.structure),2:ncol(pls)]))[5],"" )) )
        df$exact.structure.6 <- unlist(lapply(df$structure,function(x) ifelse(x %in% pls$gen.structure,unname(unlist(pls[match(x,pls$gen.structure),2:ncol(pls)]))[6],"" )) )
        df$exact.structure.7 <- unlist(lapply(df$structure,function(x) ifelse(x %in% pls$gen.structure,unname(unlist(pls[match(x,pls$gen.structure),2:ncol(pls)]))[7],"" )) )
